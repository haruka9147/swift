...
let yourSentence = "\(nounArray[1]) was broken by spiltted \(nounArray[8])"
let yourSillySentence = "\(nounArray[index1]) was broken by spiltted \(nounArray[index2])"
//: ### Exercise 2
...
var arr = newestString.split(separator: " ")
for word in arr {
    if("monkey" == word) {
        output += "🐒 "
    } else if ("iPhone" == word) {
...
print("$" + String(Double(numOfPennies) / 100))
//desired output = "$5.67"
//: # Let or Var?
import UIKit
import Foundation
