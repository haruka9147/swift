...
for (food, yum) in polarizingFoods {
    if yum {
        print("\(food), yum!")
    } else {
        print("\(food), yuck!")
...
let move = (myMove, yourMove)
switch move {
case (let my, let your) where my == .rock && your == .paper || my == .paper && your == .rock:
    resultsMessage = "Paper covers Rock."
case (let my, let your) where my == your:
    resultsMessage = "It's a tie!"
case (let my, let your) where my == .rock && your == .scissors || my == .scissors && your == .rock:
    resultsMessage = "Rock crushes Scissors. "
case (let my, let your) where my == .paper && your == .scissors || my == .scissors && your == .paper:
    resultsMessage = "Scissors cut Paper."
default: break
}
//: ### Exercise 7
...
//}
let pigLatin = (vowels, firstLetter)
switch pigLatin {
case (let arr, let c) where arr.contains(c):
    newWord = word + "yay"
default:
    word.remove(at: word.startIndex)
    newWord = "\(word)\(firstLetter)ay"
