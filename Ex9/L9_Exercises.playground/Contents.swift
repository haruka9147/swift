...
let reverseOrder = surnames.reversed()
print(Array(reverseOrder))
...
let sortedAverages2 = battingAverages.sorted(by: { $1 > $0 })
...
let result3 = particleSizesInMicrons.filter({$0 > 20})
print(result3)
//: __Problem 5__
//:
...
let priceInDollars = pricesInRupees.map({ (rupee: Int) -> Double in
    return  (Double(rupee) * 0.014).rounded()
})
...
let goalTimes = oldTimes.map({ (time: String) -> String in
    return String(timeStringFromInterval(timeIntervalFromString(time) + 13))
})
